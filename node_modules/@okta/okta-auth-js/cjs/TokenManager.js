...
function validateToken(token) {
  if (!(0, _util.isObject)(token) || !token.scopes || !token.expiresAt && token.expiresAt !== 0 || !(0, _types.isIDToken)(token) && !(0, _types.isAccessToken)(token) && !(0, _types.isRefreshToken)(token)) {
...
function setTokens(sdk, tokenMgmtRef, storage, {
  accessToken,
  idToken,
  refreshToken
  ...
  const idTokenKey = getKeyByType(storage, 'idToken') || _constants.ID_TOKEN_STORAGE_KEY;

  const accessTokenKey = getKeyByType(storage, 'accessToken') || _constants.ACCESS_TOKEN_STORAGE_KEY;

  const refreshTokenKey = getKeyByType(storage, 'refreshToken') || _constants.REFRESH_TOKEN_STORAGE_KEY; // add token to storage
  ...
  storage.setStorage(tokenStorage); // emit event and start expiration timer
...
function renew(sdk, tokenMgmtRef, storage, key) {
  ...
  clearExpireEventTimeoutAll(tokenMgmtRef); // A refresh token means a replace instead of renewal
  ...
  tokenMgmtRef.renewPromise[key] = sdk.token.renewTokens({
    ...
    setTokens(sdk, tokenMgmtRef, storage, freshTokens, (accessTokenKey, accessToken) => emitRenewed(tokenMgmtRef, accessTokenKey, accessToken, oldTokenStorage[accessTokenKey]), (idTokenKey, idToken) => emitRenewed(tokenMgmtRef, idTokenKey, idToken, oldTokenStorage[idTokenKey]) // not emitting refresh token as an internal detail, not a usable token
    ...
    if (err.name === 'OAuthError' || err.name === 'AuthSdkError') {
      ...
      storage.setStorage(tokenStorage); // emit removed events

      if (!removedTokens.length) {
        // tokens have been removed from other tabs
        // still trigger removed event for downstream listeners
        emitRemoved(tokenMgmtRef, _constants.ID_TOKEN_STORAGE_KEY);
        emitRemoved(tokenMgmtRef, _constants.ACCESS_TOKEN_STORAGE_KEY);
      } else {
...
class TokenManager {
  ...
  constructor(sdk, options) {
    ...
    var clock = _clock.default.create();
    ...
    this._getOptions = () => (0, _util.clone)(options);
    ...
    const onTokenExpiredHandler = key => {
      if (options.autoRenew) {
        if (shouldThrottleRenew(renewTimeQueue)) {
          const error = new _AuthSdkError.default('Too many token renew requests');
    ...
    setExpireEventTimeoutAll(sdk, tokenMgmtRef, storage); // Sync authState cross multiple tabs when localStorage is used as the storageProvider
